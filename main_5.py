...
def generate_meta_dataset(index):
    ...
    filehandler = open(f"./label_dict/2/multi_label_dict.pickle", "wb")
    pickle.dump(mdg.get_label_dict(), filehandler)
    filehandler.close()

    filehandler = open(f"./metadata_multi_label_matrices/2/metadata_multi_label_matrix_{index}.pickle", "wb")
    pickle.dump(metadata_multi_label_matrix, filehandler)
    filehandler.close()

    df_meta_data.to_csv(f'./metadataframes/2/meta_dataframe_{index}.csv')
